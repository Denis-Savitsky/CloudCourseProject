...
trait AuthService[F[_]] {
  ...
  def findToken(token: String): F[TokenSearchResponse]
...
class AuthServiceImpl(redis: RedisCommands[IO, String, String]) extends AuthService[IO] {
  ...
  override def findToken(token: String): IO[TokenSearchResponse] = {
    for {
      res <- redis.get(token.split("-").headOption.getOrElse(""))
      status = if (res.contains(token)) AuthStatus.NotFound else AuthStatus.Found
